...
from app.schemas.payment_schemas_v1 import PaymentReqV1, PaymentResV1
...
class PaymentServiceV1():
    ...
    def create(self, expense_id: int, new_payment: PaymentReqV1) -> PaymentResV1:
        try:
            ...
            return PaymentResV1.model_validate(payment)
        except Exception as ex:
            logger.error(type(ex))
            logger.critical(ex.args)
            raise ex

    def create_subscription_payment(self, expense_id: int, new_payment: PaymentReqV1) -> PaymentResV1:
        self.__check_expense_type(expense_id, ExpenseTypeEnum.SUBSCRIPTION)
        return self.create(expense_id, new_payment)

    def get_many(self, search_filter: dict = {}) -> List[PaymentResV1]:
        try:
            payments = self.repo.get_many(search_filter=search_filter)
            return [PaymentResV1.model_validate(payment) for payment in payments]
        except Exception as ex:
            logger.error(type(ex))
            logger.critical(ex.args)
            raise ex

    def get_by_id(self, payment_id: int, search_filter: dict = {}) -> PaymentResV1:
        try:
    ...
    def update(self, payment_id: int, payment: PaymentReqV1, search_filter: dict = {}) -> PaymentResV1:
        try:
            search_filter.update(id=payment_id)
            updated_payment = self.repo.update(
                payment.model_dump(exclude_none=True), search_filter
            )
            return PaymentResV1.model_validate(updated_payment)
        except re.NotFoundError as err:

...
from app.repositories.expense_repository_v1 import ExpenseRepositoryV1 as ExpenseRepo
from app.repositories.payment_repository_v1 import PaymentRepositoryV1 as PaymentRepo
from app.schemas.expense_schemas_v1 import NewExpenseReqV1, ExpenseResV1
from app.schemas.expense_schemas_v1 import UpdateExpenseReqV1
...
class ExpenseServiceV1():
    ...
    def create(self, user_id: int, new_expense: NewExpenseReqV1) -> ExpenseResV1:
        try:
    ...
    def get_many(self, order_by: str = 'id', order_asc: bool = False, search_filter: dict = {}) -> List[ExpenseResV1]:
        try:
            ...
            expense_list = [ExpenseResV1.model_validate(expense) for expense in expenses]
    ...
    def get_by_id(self, expense_id: int, search_filter: dict = {}) -> ExpenseResV1:
        try:
            search_filter.update(id=expense_id)
            expense = self.repo.get_one(search_filter)
            response = ExpenseResV1.model_validate(expense)
    ...
    def update(self, expense_id: int, expense: UpdateExpenseReqV1):
        try:
            ...
            response = ExpenseResV1.model_validate(updated_expense)
    ...
    def set_enable(self, expense_id: int, enabled: bool, from_endpoint: bool = True):
        try:
            ...
            if from_endpoint:
                self.__check_expense_type(ExpenseResV1.model_validate(expense), ExpenseTypeEnum.SUBSCRIPTION)
            status = ExpenseStatusEnum.ACTIVE if enabled else ExpenseStatusEnum.FINISHED
            updated_expense = self.repo.update(
                {'status': status}, {'id': expense_id})
            return ExpenseResV1.model_validate(updated_expense)
        except re.NotFoundError as err:
            raise ce.NotFound(err.message, 'EXPENSE_NOT_FOUND')
        except Exception as ex:
            logger.error(type(ex))
    ...
    def __check_expense_type(self, expense: ExpenseResV1, expense_type: ExpenseTypeEnum):
        try:
    ...
    def __update_payments_amount(self, updated_expense: ExpenseResV1):
    ...
    def update_expense_status(self, expense_id: int) -> None:
        expense: ExpenseResV1 = self.get_by_id(expense_id)

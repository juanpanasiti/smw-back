...
from app.schemas.payment_schemas_v2 import NewPaymentReqV2, UpdatePaymentReqV2
...
class ExpenseServiceV2:
    ...
    def payment_service(self):
        if self.__payment_service is None:
            self.__payment_service = PaymentServiceV2()
        return self.__payment_service

    def create(self, new_expense: NewExpenseReqV2) -> ExpenseResV2:
        ...
        expense_res = ExpenseResV2(**expense_dict)
        self.__create_installments(expense_res)
        return expense_res

    def get_list(self, user_id: int | None, account_id: int | None, params: ExpenseListParamV2) -> List[ExpenseResV2]:
        ...
        return [ExpenseResV2(**expense) for expense in expenses]

    def get_one(self, search_filter: dict, include_relationships: bool) -> ExpenseResV2 | None:
        expense = self.expense_repo.get_one(search_filter, include_relationships)
        return ExpenseResV2(**expense) if expense else None

    def update(self, expense: UpdateExpenseReqV2, search_filter: dict) -> ExpenseResV2 | None:
        new_data = expense.model_dump(exclude_none=True)
        updated_expense = self.expense_repo.update(new_data, search_filter)
        expense_res = ExpenseResV2(**updated_expense)
    ...
    def __create_installments(self, expense: ExpenseResV2):
    ...
    def __update_installments_amount(self, expense: ExpenseResV2):
        remaining_amount = expense.amount
        remaining_installments = expense.installments
        for payment in expense.payments:
            if payment.status not in FINISHED_PAYMENT_STATUSES:
                ...
                self.payment_service.update(UpdatePaymentReqV2(amount=new_payment_amount), {'id': payment.id})
